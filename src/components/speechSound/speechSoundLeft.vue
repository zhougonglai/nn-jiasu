	...
	<div class="speech-sound-left-wrap">
		...
		<div
			:class="[
				...
				{ 'speech-sound-catalogue-active': item.choosen },
			]"
			v-for="(item, index) in categoryList"
			:key="index"
			@click="chooseSpeechSoundCategory(item)"
		>
			<div class="speech-sound-title">
				<img :src="item.icon" />
				<span>{{ item.name }}</span>
			...
			<div class="speech-channel-list">
				<div
					...
					:key="index2"
					class="speech-channel-item"
				>
					<span :class="{ 'channel-choosen': item.choosen && index2 === 0 }">{{
						item2.name
					}}</span>
				</div>
			</div>
